...
public interface IPoolHistorySyncRepositoryFacade
{
    ...
    /// </summary>
    /// <param name="positions">A collection of <see cref="LiquidityPoolPosition"/> entities representing the pool positions to merge.</param>
    /// <param name="snapshots">A collection of <see cref="LiquidityPoolPositionSnapshot"/> entities representing the snapshots to merge with the positions.</param>
    /// <param name="ct">An optional cancellation token to cancel the operation if required.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    Task MergePoolPositionsAsync(IList<LiquidityPoolPosition> positions,
